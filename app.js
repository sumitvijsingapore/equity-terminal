...
const State = {
  ...
  portfolio: JSON.parse(localStorage.getItem("terminal_portfolio")||"{}"),
  compare: [],
  playbookCat: "all",
  playbookMkt: "ALL",
  playbookOpen: null,
...
function savePortfolio(){ localStorage.setItem("terminal_portfolio", JSON.stringify(State.portfolio)); }
...
function parseCsv(text){
  ...
  const keys=rows.shift().map(v=>v.trim().toLowerCase().replace(/[^a-z0-9]/g,""));
...
async function importPortfolioFile(input){
  ...
  try{
    ...
    for(const r of records){
      const t=String(r.ticker||r.symbol||r.stock||r.code||"").trim().toUpperCase().replace(/[^A-Z0-9.\-&]/g,"");
      if(!t){skipped++;continue;}
      const market=String(r.market||r.exchange||r.country||"").toUpperCase();
      const mkt=(market.includes("IN")||market.includes("NSE")||market.includes("BSE"))?"IN":"US";
      const quantity=Number(r.quantity||r.qty||r.shares||r.units)||0;
      const buyPrice=Number(r.buyprice||r.purchaseprice||r.averageprice||r.avgprice||r.costprice)||0;
      const price=Number(r.currentprice||r.price)||null;
      const listed=State.data.find(s=>s.t.toUpperCase()===t);
      if(!listed){
        const stock={t,n:String(r.companyname||r.name||t).trim()||t,mkt,price};
        const i=State.customStocks.findIndex(s=>s.t===t);
        if(i>=0) State.customStocks[i]={...State.customStocks[i],...stock}; else State.customStocks.push(stock);
      }
      const key=listed?listed.t:t;
      if(!State.watchlist.includes(key)) State.watchlist.push(key);
      if(quantity||buyPrice) State.portfolio[key]={quantity,buyPrice};
      added++;
    }
    saveCustomStocks(); saveWatchlist(); savePortfolio();
    alert(`Imported ${added} stock${added===1?"":"s"}${skipped?`; skipped ${skipped} row${skipped===1?"":"s"} without a ticker`:""}.`);
    render();
  }catch(err){ alert(err.message||"Could not import this file."); }
}
...
function allStocks(){
  const listed=new Set(State.data.map(s=>s.t));
  const custom=State.customStocks.filter(s=>!listed.has(s.t)).map(s=>({
    ...s, sec:"Custom", ind:"User added", shares:null, mcap:null, ev:null, debt:null, g:0,
    annual:{periods:[],revenue:[],ebitda:[],netIncome:[],fcf:[],ocf:[],capex:[],operatingIncome:[],grossProfit:[],margins:[]},
    quarterly:{periods:[],revenue:[],ebitda:[],netIncome:[],fcf:[],ocf:[],capex:[],operatingIncome:[],grossProfit:[],margins:[]}
  }));
  return State.data.concat(custom);
}
...
function renderHeader(){
  ...
  <header class="app">
    <div class="headrow">
      ...
      <span class="livebadge ${State.live?'live':'sample'}">${State.live?'● LIVE · YAHOO':'● SAMPLE DATA'} · ${allStocks().length} names</span>
    </div>
  </header>`;
}
    ...
    <tbody>
      ...
      rows.map(s=>`
        <tr class="row" data-open="${s.t}">
          ...
          <td class="left"><span class="tname">${s.t}${s.sec==="Custom"?` <span class="customtag">CUSTOM</span>`:""}</span><span class="tsub">${s.n}</span></td>
...
function renderWatchDrawer(){
  ...
  const custom = State.customStocks.filter(s=>State.watchlist.includes(s.t)&&!State.data.some(d=>d.t===s.t));
  return `
  <div class="watchlist show">
    ...
    <details class="bulkimport">
      <summary>+ Import portfolio (Excel / CSV)</summary>
      <div class="importbox">
        <input id="portfolioFile" type="file" accept=".csv,.xlsx,.xls,text/csv" />
        <p class="customnote">Columns: <b>Ticker</b>, Market, Company Name, Quantity, Buy Price, Current Price. Only Ticker is required.</p>
      </div>
    </details>
  </div>`;
}
...
function wireEvents(){
  ...
  if(portfolioFile) portfolioFile.onchange=()=>importPortfolioFile(portfolioFile);
